...
A comprehensive statistics tool for analyzing JSONL files containing text data is available in [`tools/stat_jsonl.py`](tools/stat_jsonl.py).
...
**Basic Counts:**
- Total files, lines, valid lines, empty lines, error lines
- Missing or empty text fields

**Content Statistics:**
- Character count (total and average per line)
- Word count (total and average per line)
- Sentence count (total and average per line)
- Token count (using SentencePiece model, optional)
- UTF-8 byte count
...
- Alphabetic characters (count and percentage)
- Digit characters (count and percentage)
- Punctuation characters (count and percentage)
- Whitespace characters (count and percentage)
- Chinese characters (count and percentage)
- Other characters (count and percentage)
...
python tools/stat_jsonl.py ./data

# Show detailed per-file statistics
python tools/stat_jsonl.py ./data --details

# Use with SentencePiece model for token statistics
python tools/stat_jsonl.py ./data --model ./model.model

# Save results to JSON file
python tools/stat_jsonl.py ./data --output results.json

# Combine options
python tools/stat_jsonl.py ./data --model ./model.model --details --output results.json
```
